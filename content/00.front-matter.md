...
+ **{{author.name}}**
  {% if author.equal is defined and author.equal == true -%}^[☯](#equally)^{%- endif -%}
...
::: {#equally}
✉ — {% if manubot.authors|map(attribute='equally')|select|max -%}
{% for author in manubot.authors|selectattr("equally") -%}
{{ author.name }} \<{{ author.email }}\>{{ "and " if not loop.last else " contributed equally to this work." }}
{% endfor %}
{% endif %}
:::
